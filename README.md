...
`tools/list` is a stable discovery catalog for the authenticated account role. A brief relay interruption does not withdraw tool definitions or require a tools-list-changed notification. Discovery is not authority: every `tools/call` is still intersected with the current end-to-end-ready daemon policy and tool ceiling, and fails retryably with `unavailable` when no daemon is ready. `server_info.tool_delivery` distinguishes the stable advertised catalog from the currently effective daemon/account intersection. Hosted synchronous calls deliberately reserve reply-settlement headroom: ordinary daemon tools default to 20 seconds of execution plus a separate five-second Worker settlement margin, while configurable browser/application foreground tools default to 20 seconds and retain a 45-second explicit maximum. Remote `exec_command`, `run_process`, and `run_local_command` require a caller-held `idempotency_key`, then commit a principal-bound one-step managed job, return its `job_id` within a 10-second acceptance budget, and let the detached step run for up to 600 seconds; the key makes an ambiguous acceptance retry converge on the same retained job, while `read_job` recovers progress/results across relay loss or daemon replacement after acceptance. `start_process` remains the request-owned interactive daemon-lifetime path: remote startup has a 10-second execution budget plus the settlement margin and performs resource admission without queueing by default, while owner-local sessions retain the cooperative admission wait. `read_process` waits at most five seconds per remote poll. The durable process façade preserves the original account/tool authority and delegated workspace sandbox; it does not grant non-owners the owner-only multi-step `start_job` surface.
