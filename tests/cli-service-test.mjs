...
const command = createServiceCommand({
  ...
  service,
  inspectWorkspaceDaemon: () => ({ alive: true, verified_service_daemon: true, pid: 42 }),
...
assert(calls.some((entry) => Array.isArray(entry) && entry[0] === "start"), "start did not call the provider adapter");
...
const failingStart = createServiceCommand({
  ...
  stateRootFromArgs: () => "/synthetic-state",
  structuredLogger: () => ({}),
  service: { ...service, async startAutostart() { return { ok: false, active: false, reason: "synthetic" }; } },
  selectedWorkspace: () => null,
...
try {
  ...
  const defaults = createServiceCommand({
    ...
    stateRootFromArgs: () => "/synthetic-state",
    structuredLogger: () => ({}),
    service: { ...service, async startAutostart() { return { ok: false, active: false, reason: "default-adapter-test" }; } },
    selectedWorkspace: () => null,
  });
