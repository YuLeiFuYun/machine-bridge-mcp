...
async function resourceCliSelfTest() {
  ...
  const entry = fileURLToPath(new URL("../bin/machine-mcp.mjs", import.meta.url));
  try {
    const added = spawnSync(process.execPath, [entry, "resource", "add", "racknerd-key", resourceFile, "--workspace", workspace, "--state-dir", stateRoot, "--json"], {
    ...
    const stagedForCli = manager.stage({
      name: "CLI approval",
      steps: [{ argv: [process.execPath, "-e", "require('node:fs').writeFileSync(process.argv[1],'cli-approved')", approvedMarker], env_resources: { MBM_REVIEW_ONLY: "racknerd-key" }, timeout_seconds: 10 }],
    });
    ...
    const reviewedResource = inspectionJson.review_plan?.resources?.["racknerd-key"];
    ...
    const removed = spawnSync(process.execPath, [entry, "resource", "remove", "racknerd-key", "--workspace", workspace, "--state-dir", stateRoot, "--json"], {
      encoding: "utf8", timeout: 10_000,
    });
