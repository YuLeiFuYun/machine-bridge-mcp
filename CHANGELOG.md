...
- Reduce persistence-domain coupling. Static package identity moved to `package-identity.mjs`, pure state path/hash calculations moved to `state-locations.mjs`, and stateless browser/shell/health/CLI consumers no longer depend on `state.mjs` merely for constants or path helpers. The source import graph remains acyclic while direct `state.mjs` fan-in fell from roughly 30 modules at review start to 17.
