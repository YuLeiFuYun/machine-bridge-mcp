...
- Worker health direct/proxy routing through a real local HTTP CONNECT proxy, `NO_PROXY` bypass, redirect/body/deadline bounds, error classification, bounded propagation retry, and invalid proxy fail-fast behavior without endpoint or credential disclosure;
