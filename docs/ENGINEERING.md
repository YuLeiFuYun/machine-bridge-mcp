...
18. **Durable state owners do not retain cross-event terminal Promises or depend on JavaScript timers as durable ownership.** Legacy recoverable stream initiation validates and transactionally persists stream plus daemon-call ownership before dispatch; later events converge through one guarded terminal write. The Durable Object alarm is the persistent deadline authority. Ordinary HTTP/WebSocket events may run a bounded durable recovery sweep only when the persisted alarm is missing, already due, or unreadable; they must not enumerate every retained stream on each event merely as a compensating timer. New durable-call admission commits the active stream row and advances the persisted alarm to no later than its operation deadline inside the same storage transaction, so crash consistency does not depend on a later scheduler callback or a compensating full-table scan. Its descriptor and hibernatable subscription paths are internal-only. Modern streams are different: one Durable Object fetch owns the direct response, the outer Worker keeps it observable with bounded SSE comments, and a random private capability indexes only the already-active pending call. Public control headers are stripped, cancellation carries no OAuth/DPoP credential, and modern code must never add prepare/subscribe descriptors, terminal-result retention, or a cross-event Promise registry.
...
- Pure static/package identity and state-location computation stay outside persistence ownership. `package-identity.mjs` owns installed package name/version/root plus the shared server name; `state-locations.mjs` owns home expansion, state-root/profile/resource/service-control paths, and workspace hashing. `state.mjs` may consume/re-export those values for compatibility, but modules that need only those pure values must depend on the narrower boundary rather than the persistence/lock domain.
