...
- Exclusive locks use the shared complete-before-visible hard-link claim. Ownership bytes plus device/inode/size/mtime evidence come from one opened descriptor snapshot; ownership locks reject multiple hard links, and release/reclamation rechecks the path identity/token before unlink. Do not combine bytes from one open with identity from a prior path stat or unlink a path merely because an earlier read looked stale.
