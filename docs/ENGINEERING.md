...
18. **Durable state owners do not own long client streams.** A Durable Object that must accept cancellation, status, or recovery requests cannot retain the public SSE response or any internal request waiting for terminal completion. The outer Worker owns streaming; Durable Object descriptor and poll requests are short, immediate, authenticated at admission, and unreachable through caller-supplied internal headers.
