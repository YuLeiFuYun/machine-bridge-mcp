...
The accepted prerelease's Git tag and GitHub Prerelease require explicit owner authorization and use `npm run prerelease:release -- --owner-confirm`. A TTY is optional; the command may also run from an explicitly authorized local automation/Machine Bridge session. npm publication remains a separate explicit owner decision through `npm run prerelease:publish`. Exact registry installation/activation likewise requires explicit owner authorization and uses `npm run prerelease:install -- --allow-worker-deploy`. Formal soak begins only from this registry-verified activation. Minimum soak is seven days for major, three days for minor, and one day for patch releases. A blocking defect increments the prerelease number and restarts the soak interval.

Stable promotion requires a tracked `release-soak/v<stable>.json` and identical promotion-content digest. Only normalized release metadata may differ from the soaked prerelease. The stable candidate is activated and observed again before explicitly authorized Git tag/GitHub Release publication through `npm run release -- --owner-confirm`; npm stable publication remains a separate explicit owner decision through `npm run stable:publish`.
...
For a known managed job whose result is required by the current task, an active relay-origin `read_job` uses `status_polling_mode=bounded_followup` with `host_turn_handoff_recommended=false`, and the client may issue another `read_job` in the same assistant response. **Autonomy must not be implemented as a host-side spin loop or as one overlong host call:** active hosted `read_job` uses the advertised 40-second server-side long-poll by default, returning early on meaningful status/phase progress or terminal state; `wait_ms=0` is an explicit immediate checkpoint, not the normal wait strategy, while a client with independently verified longer request lifetime may explicitly request up to five minutes. The default must remain below any empirically observed host tool-call termination boundary rather than being raised merely to reduce call count. Per-call survival and aggregate host-response lifetime are separate constraints: arithmetic such as `duration / wait interval` is useful for estimating interaction density but is **not** evidence that one assistant response can carry every read to terminal state. Process sessions use the corresponding `paced_followup` model: after a blocking read arms the fifteen-second cooldown, another would-block request is paced inside that same MCP call until output/exit or the cooldown boundary rather than returned as a rapid running checkpoint. The managed-job server-side long-poll, remote one-second actual output/exit blocking `read_process` cap, fifteen-second cooldown, `next_blocking_poll_after_ms`, durable ownership, idempotency, and actual request/transport limits are the anti-amplification controls; a mandatory one-read-per-response, rapid immediate-checkpoint loop, later-user-turn boundary, or guessed elapsed-time cutoff is not an acceptable substitute for those controls.
...
The execution lifetime must also support the continuity claim below the status layer. Remote one-step process tools remain intentionally capped at 600 seconds; a continuous process that legitimately needs longer must route to an owner-authorized managed-job step. Main/finally steps default to 600 seconds and may explicitly request up to 21,600 seconds (six hours), with cooperative resource admission occurring before that execution timer starts. Do not split a stateful long command merely to work around an orchestration timeout. The local plan validator and all published job schemas must share the same ceiling. Completed one-step process carriers may share the bounded managed-job store, but they are lower-priority terminal retention than explicit managed jobs: when capacity pressure has removable transient helper history, helper churn must reclaim that history before evicting an explicit managed-job recovery result. The hard retained-state cap and time-based privacy retention remain authoritative.
