...
**Job polling ownership is not task ownership.** An MCP Apps-capable hosted `start_job` may advertise `ui_monitor_candidate=true` plus `ui_monitor_render_tool=render_job_monitor` and a fresh random `ui_monitor_id`, but that is only an optional status surface: `start_job` remains a data/execution tool and carries no UI template. The model must not render the monitor merely to shorten a response, avoid waiting, or manufacture a handoff while the current task still has work that depends on the managed job's terminal result. In that case the model preserves `job_id` + `recovery_key` and continues bounded server-paced `read_job` calls in the same response while calls are accepted. The monitor is eligible only when the current task can deliberately transfer that job's status polling without needing its terminal result for remaining same-response work. When intentionally used, the model preserves the pre-issued ID, calls `render_job_monitor` once with the exact accepted `job_id` + `recovery_key` + `ui_monitor_id`, and then calls model-side `read_job` once with the same ID without depending on either structured or text render output. A matching current View claim on an ordinary job reports `ui_monitor_claimed=true`, `status_polling_mode=ui_monitor`, `host_turn_handoff_recommended=false`, and `follow_up_read_required=false`: that suppresses duplicate model polling of this job only and never instructs the assistant to abandon unrelated or still-actionable work. The only permitted host-turn exception is an explicitly persisted `continuation_mode=task_supervisor` whose reviewed durable plan already encapsulates every remaining noninteractive task action; a later active claimed read may then report `host_turn_handoff_recommended=true` because the supervisor job owns the remaining execution lifecycle. Machine Bridge can validate the persisted declaration, active state, and current monitor claim but cannot infer conversation-level completeness; that completeness remains an owner/caller semantic assertion. The monitor itself remains status-only and receives no execution or capability authority. The Worker verifies read authority, activates the pre-issued monitor record, and mounts the Job Monitor resource. New issuance is latest-wins for the same job/principal, so older monitor instances stop being current. Issued/active/claimed monitor metadata must be transactionally persisted in the BridgeRoom Durable Object, not kept only in isolate-local memory; persistence is metadata-only and must never include the recovery capability or job result. An issued ID is not claimable before render activation. After a matching claim, the monitor remains capability-bound and continues through the app-only `read_job_monitor` host bridge, which verifies the original `recovery_key`, requires the monitor ID to identify the latest claimed View, refreshes its five-minute handoff-freshness lease, strips recovery/correlation material before delegating to a fixed 40-second daemon `read_job`, and gains no bare-ID inventory or direct network path. Expired handoff freshness cannot transfer task ownership, but a still-latest View may resume app polling after host suspension or machine sleep; a replacement issuance removes the older record and prevents that older View from resuming. A transient app-origin client-to-Worker read failure is retried by the View with bounded backoff against the same durable job rather than pausing for a user turn or resubmitting execution. A read already dispatched before replacement may finish, but no later read from that replaced View is admitted.

The app-only monitor is a minimum-status continuation surface, not another managed-job result API. After the Worker checks the recovery capability and obtains the daemon `read_job` result, it must project only bounded lifecycle fields before returning to the View; stdout/stderr, step/nested result objects, command text, paths, recovery/control capabilities, and unknown fields must not cross that boundary. The View may retain the render-input `recovery_key` only as its live read authority, must bind that complete render input once rather than accepting ambient tool-result state, must not reflect raw host/runtime exceptions into UI text, and must clear capability/correlation values plus pending response timers on teardown. Durable monitor metadata remains auxiliary: a storage failure during optional issuance or claim probing after the core managed-job result exists must degrade to ordinary model-side continuation, and cleanup failure must not prevent authority revocation. Observability for those failures is limited to a fixed operation class plus sanitized error class. Persistent monitor records must remain schema-bounded and reject duplicate IDs/order sequences or impossible generation ordering.
