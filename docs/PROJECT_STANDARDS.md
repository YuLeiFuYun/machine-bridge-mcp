...
An MCP Apps-capable hosted `start_job` may advertise `ui_monitor_candidate=true` plus `ui_monitor_render_tool=render_job_monitor` and a fresh random `ui_monitor_id`, but that is not ownership transfer: `start_job` remains a data/execution tool and carries no UI template. The model preserves that ID and calls `render_job_monitor` once with the exact accepted `job_id` + `recovery_key` + `ui_monitor_id`; the Worker verifies read authority, activates the pre-issued monitor record, and mounts the Job Monitor resource. The initiating model must not depend on either structured or text output from the UI render tool; the next model-side `read_job` carries the same ID already obtained from `start_job`. Model-side continuation stops only if that exact active read reports `ui_monitor_claimed=true`, `status_polling_mode=ui_monitor`, `host_turn_handoff_recommended=true`, and `follow_up_read_required=false`, proving that a mounted View completed its handshake, observed `hostCapabilities.serverTools`, and claimed the same activated monitor instance using the original recovery capability. An issued ID is not claimable before render activation, the monitor remains capability-bound, calls only the existing hosted `read_job` through the host bridge, and gains no bare-ID inventory or direct network path. A different start result receives a different monitor ID, so an older claim cannot transfer that newer handoff. This ownership transfer avoids consuming an unobservable aggregate host-turn lifetime without guessing elapsed time or branching on a host product name.
