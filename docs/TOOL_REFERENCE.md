...
Inspect a bounded macOS Accessibility tree for a running application. Requires local Accessibility permission and never executes arbitrary AppleScript supplied by the caller. Foreground execution is limited to 60 seconds; use process sessions or managed jobs for longer work.
...
Read the tab inventory from the paired existing Chromium profile without creating, activating, or closing a tab. Use browser_manage_tabs for tab mutations. Foreground execution is limited to 60 seconds; use process sessions or managed jobs for longer work.
...
Wait until all supplied URL, load, text, and element-state conditions are satisfied in an existing browser tab. Foreground execution is limited to 60 seconds; use process sessions or managed jobs for longer work.
