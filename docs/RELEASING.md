...
Before calling any checkout “current main”, “the online version”, or a release candidate, verify its identity rather than trusting the directory name. Inspect `git worktree list --porcelain`, the current branch/HEAD, `package.json` version, and `src/shared/server-metadata.json` tool-schema generation. When comparing against a deployed runtime, also require live `server_info.version`/`tool_delivery.tool_schema_generation`; when validating a hosted ChatGPT action snapshot, load the complete unfiltered tool catalog and compare its generation as a separate host-owned surface. A mismatch is a hard review/release preflight failure. Dirty historical worktrees must be preserved or explicitly archived before removal; a canonical-looking path is never sufficient evidence that it is the main worktree.
...
The command still fails closed on dirty state, wrong branch/version channel, missing exact acceptance, unsuccessful exact-commit CI, digest mismatch, conflicting tags/releases, or concurrent publication. Missing, queued, or in-progress required push workflows for the exact `main` commit are a bounded waiting state inside the durable release command rather than a reason to stop orchestration: all five required workflows share one 30-minute monotonic deadline and are polled no more frequently than every 15 seconds. The newest exact-SHA push run is authoritative, so an older success cannot mask a newer pending or failed rerun. A completed non-success result fails immediately, and deadline expiry fails before any tag or GitHub Release mutation. This waiting behavior does not authorize npm publication; that remains the explicit owner-confirmed boundary below. Before running its frozen full verification, the command creates the integrity-pinned hardened npm session and executes lockfile-only `npm ci` against the checkout, then rechecks that the tracked tree is still clean. Its long npm lifecycle stages use process-tree hard-timeout settlement, so a deadline cannot be reported as terminal while npm descendants continue running. One common-Git-dir publication lock serializes tag/Release writes across the main checkout and linked worktrees. No TTY or owner-confirmation ceremony is part of the GitHub publication contract.

The command resolves trusted absolute git and GitHub CLI executables and treats the tracked acceptance record plus the exact merged source as publication authority; an ignored `.release-candidate/` directory is only local candidate-development state and is not required to survive a worktree/branch transition. While its integrity-pinned hardened npm session is still available, publication deterministically rematerializes the package only into a private temporary directory, requires package name/version/filename/SHA-1/SRI and the promotion-content digest to equal the current acceptance record, verifies those bytes again, and writes the exact upload copy as an owner-private file. This is not permission to regenerate different acceptance bytes: any mismatch fails before tag or Release mutation. The low-level candidate resolver used without an explicit hardened npm CLI still rejects stale or unsafe checkout-local manifest/tarball state. After upload the command queries the GitHub REST asset record and requires its SHA-256 digest to equal the current accepted artifact before reporting success.
