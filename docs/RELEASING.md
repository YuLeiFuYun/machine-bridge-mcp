...
The command still fails closed on dirty state, wrong branch/version channel, missing exact acceptance, incomplete exact-commit CI, digest mismatch, conflicting tags/releases, or concurrent publication. One common-Git-dir publication lock serializes tag/Release writes across the main checkout and linked worktrees. No TTY or owner-confirmation ceremony is part of the GitHub publication contract.
