...
Structured-field sanitizers use null-prototype intermediate maps, so prototype-shaped input keys such as `__proto__` and `constructor` remain ordinary bounded data instead of mutating the sanitizer object model. Sensitive key/value redaction is applied before serialization exactly as for other fields. The internal logger exposes unsanitized terminal delivery only through explicitly named `rawPlain` / `rawJson` methods; those methods are reserved for command results whose purpose is to return exact user-owned values such as the initial one-time owner credential or requested local connection/path output, and must not be used for operational events, exceptions, or service logs.
