...
The beta.170 idle-sleep repair is driven by separate owner-machine outages that correlated with real macOS idle/maintenance sleep. It does not reclassify the awake close-1006 incident that motivated beta.169. Unit and integration evidence must prove default-mode compatibility, `ac-continuous`/`continuous` semantics, bounded 1/5/30-second child recovery, persisted configuration, and intentional-stop cancellation. Owner-machine acceptance additionally requires a live `continuous` assertion and deliberate kill/reacquire fault injection; a multi-hour no-sleep soak remains separate long-horizon evidence and is not implied by short release verification.
