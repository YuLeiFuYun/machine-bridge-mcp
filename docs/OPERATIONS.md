...
| `capability_routing.bootstrap_observed` is false | The current local runtime has not received `session_bootstrap`. This is normal after modern `server/discover` alone; investigate only when the workflow expected an explicit refresh or legacy initialization bootstrap. |
