...
A reconnect warning is evidence of a transport outage, not proof that the daemon process exited. Compare daemon PID/process start, `connected_at`, `last_seen_at`, `runtime.relay.last_disconnected_at`, close category/code, and outage count. A system VPN/TUN may remain shown as connected while its internal route is unavailable; `server_info` reports only Machine Bridge's application-proxy choice, while `diagnose_runtime`/`doctor` additionally classify the macOS default route as `tunnel-or-vpn`, `physical-or-other`, `loopback`, or `other`. The classification is coarse and does not expose an interface name, address, DNS answer, endpoint, or credential. The reconnect schedule tops out at fifteen seconds.
...
A foreground MCP tool is not a durable job. Hosted clients may impose a request ceiling below the local/stdio tool schema; ChatGPT was observed abandoning a foreground call near 100 seconds. The remote Worker catalog therefore advertises at most 85 seconds while preserving each tool’s 30- or 60-second default, reserving five seconds for terminal delivery. A request above 85 seconds is rejected before daemon dispatch and reports `side_effects_started=false`; it is never silently shortened. Use `start_process` plus bounded `read_process`, or a managed job, for longer work. If the host exposes only `exec_command`, keep mutation and validation in independently terminal calls and use an ignored project-local status/output file for bounded polling.
