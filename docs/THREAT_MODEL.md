...
For delegated process execution, Machine Bridge requires a behaviorally verified OS sandbox. If the negative boundary cannot be demonstrated, delegated execution fails closed. Owner execution is not sandboxed by Machine Bridge. Fixed implementation-owned Git operations are not caller-selected process authority: they use code-constructed argv, no shell, literal pathspecs, an isolated minimal environment, bounded deadlines/output, and ordinary cancellation/process tracking. Fixed UI launch/JXA/native-input helpers likewise use code-constructed argv rather than caller-selected script source. For those mutation-capable helpers a failure before process start remains definite; once process start is established, timeout, cancellation, process error, or lost settlement is non-retryable unknown evidence because the UI mutation may already have happened. Before a restricted principal reaches Git, filesystem-first discovery checks metadata roots and recursive alternate stores and rejects symbolic-link/special-file descendants in the metadata trees; repository configuration that can execute helpers, redirect external files, or enable lazy object fetching is rejected. This protects the authenticated principal boundary but is not a hostile same-UID filesystem transaction: another local process with the owner's OS authority can still race metadata after inspection.

### Local runtime to browser extension

The broker is loopback-only, but loopback is not treated as a per-user authentication boundary. Long-lived extension/runtime tokens stay in owner-only state and serve only as HMAC keys: already-paired clients first prove possession of the role key with an init HMAC before the broker allocates challenge state, then verify a broker proof before WebSocket upgrade and present a five-second one-time role-bound proof. The fixed public pairing URL and HTTP document carry no credential. An explicit pair action first binds a one-shot OS-random loopback listener owned by the current process, then places a 30-second bootstrap only in that temporary URL fragment; the `document_start` content script strips it before page scripts, and the broker must prove knowledge derived from the owner-only extension token before the extension accepts that token. Successful bootstrap exchanges are consumed once. Extension origin, protocol/build version, capabilities, owner/client route, and handshake state are also validated. A same-OS-user malicious process remains outside the repository-only isolation guarantee, and process/URL UI visibility still relies on the operating system and browser boundary.

The extension still operates inside a real browser profile. It can reach tabs, authenticated sessions, and origins granted by browser permissions. Only the `owner` role may use browser-session control or export local data into it. Computer Use does not increase that role ceiling: its browser observations bind semantic refs, document/frame epochs, optional screenshot evidence, and private backend-node identity to one bounded snapshot, then consume that snapshot as one-shot mutation authority. Transport loss after a browser mutation is attempted is explicitly unknown and never safe replay evidence. Screenshot-bound point actions additionally require exact current screenshot/viewport/document evidence; callers cannot submit raw CDP methods, backend-node IDs, or coordinates that are not bound to the referenced snapshot.
...
Machine Bridge uses bounded reads, canonical path resolution, owner-only modes where supported, atomic replacement, process-identity locks, schema validation, and explicit lifecycle transitions. Security/destructive inventory treats only `ENOENT` as absence, refuses symbolic-link profile/job roots before traversal, and treats malformed/wrong-type entries inside reserved `profiles/`, public `job_...`, or retired cleanup namespaces as blockers rather than disposable unknown files. Whole-directory managed-job and state-root deletion is generation-bound: the exact inspected directory is moved to a reserved quarantine namespace, the moved filesystem identity and domain safety evidence are revalidated, and only that generation may be recursively deleted; crash residue is reclaimed only when the encoded generation still matches. Generic path-based tools cannot target Machine Bridge control-plane roots, even under `owner/full`. An authorized owner shell remains equivalent to same-user code execution and is outside that path boundary.
...
- an intermediary cannot authorize or route one current method/name while the Worker executes another body; mirrored-header mismatch is rejected before dispatch;
- JSON-RPC request IDs are not global identities across clients sharing one bearer token; response-stream cancellation instead uses a request-scoped unguessable internal capability that is never accepted from the public caller;
...
- cancellation, timeout, disconnect, replacement, revocation, and shutdown have explicit process ownership and cleanup semantics; an asynchronous child `error` does not by itself release tracker/resource ownership before close/exit settlement, post-spawn start failures retain internal ownership until close, and a failed direct termination request cannot be reported as a successful forced kill or completed authority revocation; streamed calls persist ownership and deadlines across Durable Object hibernation, and per-WebSocket generations reject delayed results or close events from an obsolete connection;
...
UI state can change between inspection and action. Pages can present deceptive content, and authenticated profiles may contain high-value sessions. Protocol checks and trusted-input handling do not make arbitrary web content trustworthy. Computer Use narrows stale-target risk through one-shot snapshot authority, browser document/frame/semantic identity, and macOS PID-generation/owner-window revalidation, but it cannot make a multi-command visual/semantic observation atomic when the underlying OS/browser does not provide such a primitive.

Screenshots are user data and may contain credentials, personal information, private documents, or transient notifications even when semantic value fields are redacted. They are returned only as requested tool results and are not operational log data. Computer Use shares the ordinary 7 MiB tool-result boundary; an image that would overflow the result is omitted before publishing the observation snapshot, and the resulting snapshot loses pixel-action authority. This avoids an inaccessible snapshot ID while not pretending that an omitted image was never captured in local memory during the bounded operation.

The macOS experimental background visual backend uses fixed native code and exact process/window/screenshot bindings, but it depends on Accessibility/Automation/Screen Recording permissions and private WindowServer/SkyLight behavior that Apple does not guarantee as a stable public API. It is disabled unless explicitly configured and successfully probed. Probe failure must degrade to supported semantic/screenshot evidence rather than enabling pixel input. Same-UID local interference, OS compositor races, or a UI that changes after the last preflight remain residual risks; unknown outcomes require observation before any new mutation.
...
The current runtime uses cooperative admission to reduce accidental cross-agent resource storms, protected backfill and Linux PSI pressure evidence, and project contention scoped to provable shell working directories. None of these is a hostile-process containment boundary. A same-user process can ignore the coordinator, forge unrelated workload outside Machine Bridge, deliberately escape a process group, or consume more than its advisory reservation. The coordinator therefore protects cooperating Machine Bridge/workflow roots from accidental CPU/I/O/memory/disk oversubscription and starvation; account/container/VM/cgroup controls remain required against malicious or uncooperative local code. Persistent coordinator records contain no command text or raw project path, but their existence and coarse resource-family/timing metadata are local side channels to the same OS user. PSI reports system/cgroup stall history, not which particular cooperating process caused it.
...
GitHub tag/Release publication is an integrity and serialization boundary, not a user-presence ceremony. Standing repository authority may execute it after the exact accepted candidate, `main`/remote identity, required exact-commit workflows, package/release reconciliation, and the common-Git-dir process-identity publication lock all pass. npm registry publication is the sole explicit current-task owner authorization boundary and requires the `--owner-confirm` entrypoint flag. Neither boundary provides cryptographic separation from arbitrary code already executing as the same OS user; a protected release environment, hardware-backed user presence, or a separately isolated principal is required when adversarial same-user separation is part of the threat model.
...
- account roles, trusted clients, account-version revocation, refresh-family rotation/replay, DPoP proofs, and non-escalatable effective authority; request-scoped stream cancellation does not weaken that boundary because the outer Worker strips caller-supplied internal control headers, mints a random capability itself, sends no bearer token or DPoP proof on the private cancellation request, and binds the capability only to the currently indexed in-memory call.
