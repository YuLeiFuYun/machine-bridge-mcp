...
MCP `2026-07-28` response streams are request-scoped, not persisted, and not resumable by the client. Closing a public response stream cancels that request and releases its transient Worker ownership. The Worker's pending-call registry is in memory and does not persist tool arguments or terminal results as protocol replay state.
