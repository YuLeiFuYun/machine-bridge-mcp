...
Before describing a checkout as current `main`, the online version, or a release candidate, verify identity instead of trusting the directory name. Inspect `git worktree list --porcelain`, the current branch/HEAD, `package.json` version, and `src/shared/server-metadata.json` tool-schema generation. If comparing with a deployed runtime, also verify live `server_info.version` and `tool_delivery.tool_schema_generation`; if hosted schema behavior matters, load the complete unfiltered `machine-mcp` tool catalog as the separate host-owned snapshot. A mismatch blocks the readiness claim. Never delete or repurpose a dirty historical worktree merely to make the directory layout look canonical; preserve or archive its uncommitted state first.
...
- An MCP Apps-capable active `start_job` may first report `ui_monitor_candidate=true` plus `ui_monitor_render_tool=render_job_monitor`; that is not yet a handoff. Call `render_job_monitor` once with the exact accepted `job_id` + `recovery_key`. Its result returns a fresh non-authority `ui_monitor_id` and mounts the Job Monitor View. Then call `read_job` with the same `job_id`, `recovery_key`, and `ui_monitor_id`. Continue ordinary model-side reads unless that exact active read reports `ui_monitor_claimed=true`, `status_polling_mode=ui_monitor`, `host_turn_handoff_recommended=true`, and `follow_up_read_required=false`. Those fields are emitted only after the mounted View completed its MCP Apps handshake, observed `hostCapabilities.serverTools`, and claimed the same random monitor instance using the original recovery capability. At that point the current View owns `read_job` continuation and the agent must finish the assistant response rather than duplicate those reads. A stale claim from an older View cannot transfer a new turn because its `ui_monitor_id` differs.
