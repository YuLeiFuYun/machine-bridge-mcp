...
- after all required checks pass, update the branch if necessary, squash-merge its pull request, verify `main`, and remove the merged branch;
- for every release-relevant change with an advanced package version, create and push the annotated version tag and create or update the matching GitHub Release from the exact successful `main` commit.

This is standing authorization for repository source completion. Do not leave a completed change in an open pull request or with a missing version tag or GitHub Release merely because the user did not repeat the instruction in the current task.
...
The release operator retains responsibility for live package and machine changes:
...
`release:publish` is repository-source completion and is part of the coding agent's default scope. `npm publish`, Worker deployment, credential changes, global installation, and daemon or service replacement remain explicit live-operation decisions.
