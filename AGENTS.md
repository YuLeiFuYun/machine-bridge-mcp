...
Before describing a checkout as current `main`, the online version, or a release candidate, verify identity instead of trusting the directory name. Inspect `git worktree list --porcelain`, the current branch/HEAD, `package.json` version, and `src/shared/server-metadata.json` tool-schema generation. When the owner names a specific prerelease such as `beta.N`, resolve the matching worktree **before** treating any checkout as the task target by running `node <any-worktree-of-this-repository>/scripts/resolve-versioned-worktree.mjs <any-worktree-of-this-repository> <beta.N>`. Accept only one `status=resolved` result with `fallback_used=false`. An available branch/worktree identity for the requested prerelease is stronger routing evidence than a mutable working-tree version; a dirty `package.json` version that diverges from HEAD must never be sufficient by itself to select a checkout. The canonical directory name `machine-bridge-mcp`, shortest path, current shell directory, and historical dirty worktrees are never freshness evidence or fallback targets. If the resolver reports missing, ambiguous, unavailable, or dirty-version-only evidence, stop the routing decision and reconcile the worktree inventory instead of silently choosing another checkout. If comparing with a deployed runtime, also verify live `server_info.version` and `tool_delivery.tool_schema_generation`; if hosted schema behavior matters, load the complete unfiltered `machine-mcp` tool catalog as the separate host-owned snapshot. A mismatch blocks the readiness claim. Never delete or repurpose a dirty historical worktree merely to make the directory layout look canonical; preserve or archive its uncommitted state first.
...
- **Job polling ownership is not task ownership.** An MCP Apps-capable active `start_job` may report `ui_monitor_candidate=true`, `ui_monitor_render_tool=render_job_monitor`, and a fresh non-authority `ui_monitor_id`, but the agent must not render the monitor merely to shorten or end the current response. If any remaining task-local action depends on the managed job's terminal result, preserve `job_id` + `recovery_key` and continue ordinary server-paced model-side `read_job` follow-up instead. The monitor is eligible only when the current task can deliberately transfer this job's status polling without needing its terminal result for remaining same-response work, for example an explicit user-requested background/checkpoint handoff. When the monitor is intentionally used, preserve the start-result ID, call `render_job_monitor` once with the exact accepted `job_id` + `recovery_key` + `ui_monitor_id`, and then call model-visible `read_job` once with those same values without depending on the render result being planner-visible. A matching claim on an ordinary job reports `ui_monitor_claimed=true`, `status_polling_mode=ui_monitor`, `host_turn_handoff_recommended=false`, and `follow_up_read_required=false`; those fields mean only that the mounted View owns subsequent polling for that job. The sole exception is an explicitly persisted `continuation_mode=task_supervisor`: when that durable supervisor's own reviewed plan already encapsulates all remaining noninteractive task work, a later active claimed read may report `host_turn_handoff_recommended=true`. Machine Bridge can validate the persisted continuation declaration, active job state, and current monitor claim, but it cannot infer conversation-level task completeness; that completeness remains an owner/caller semantic assertion. The assistant may then hand off the host turn because the durable supervisor owns the remaining work; the View still owns status polling only. The mounted View continues through app-only `read_job_monitor`, which verifies the same recovery capability and performs a fixed 40-second daemon `read_job`; transient app-origin read failures are retried with bounded backoff without resubmitting the job. A stale claim from an older View cannot affect a newer monitor instance because its `ui_monitor_id` differs.
- When the current task requires a hosted managed job's terminal result and no UI monitor owns continuation, the agent preserves the acceptance `job_id` and its `recovery_key`, then follows that known capability-bound job with bounded same-response `read_job` calls until it becomes terminal while calls continue to be accepted. Hosted `cancel_job` uses the separate acceptance `control_key`, and hosted `depends_on` supplies the upstream `recovery_key` through `dependency_recovery`; a bare `job_id` is not remote managed-job authority. Hosted active reads must use Machine Bridge's server-side paced long-poll rather than rapid immediate checkpoints **or one overlong host call**: the default relay read holds one MCP call for up to 40 seconds. Terminal settlement returns on the next bounded poll; nonterminal status/phase/dependency progress is coalesced for at least 30 seconds by default, and `current_step`-only churn does not wake the host call. `wait_ms=0` is reserved for an intentional immediate checkpoint, and every public hosted `read_job` call is capped at 60 seconds. Longer jobs remain autonomous by issuing another server-paced read against the same `job_id`; one longer host call is outside the current hosted contract. At the default, an unchanged 100-minute task must require no more than 150 host-visible status responses, while even continuously changing nonterminal progress must not exceed the 30-second coalescing density bound. The agent must not infer or preempt a host/tool deadline from elapsed wall-clock time, and an active read must not create an artificial one-read-per-assistant-response boundary.
...
- The agent may return an active hosted `job_id` plus its `recovery_key`, status, and current phase for later recovery only after a real host/tool/runtime boundary is actually observed, the task requires external human input or authorization, or the user explicitly requested a checkpoint rather than completion. Elapsed minutes, a guessed host budget, and "one status read already happened in this response" are never by themselves valid reasons.
- Hosted `list_jobs` is aggregate-only and deliberately does not rediscover job IDs, names, or recent recovery handles. `list_jobs`, `server_info`, `diagnose_runtime`, or another status surface must not be used to evade pacing or manufacture progress. Follow a known hosted job through `read_job` with its preserved `job_id` + `recovery_key`; use local CLI/stdio administration for legacy/pre-capability or global owner recovery.
