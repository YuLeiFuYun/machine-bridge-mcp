...
function windowsDescendantSweepScript(rootPid) {
  ...
  return `$ErrorActionPreference='Stop'; $root=${rootPid}; $roots=New-Object 'System.Collections.Generic.HashSet[int]'; [void]$roots.Add($root); for($pass=0; $pass -lt 3; $pass++){ $all=@(Get-CimInstance Win32_Process | Select-Object ProcessId,ParentProcessId); do { $changed=$false; foreach($p in $all){ $pid=[int]$p.ProcessId; $ppid=[int]$p.ParentProcessId; if($pid -ne $root -and $roots.Contains($ppid) -and $roots.Add($pid)){ $changed=$true } } } while($changed); $targets=@($roots | Where-Object { $_ -ne $root }); foreach($pid in $targets){ Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue }; Start-Sleep -Milliseconds 75 }; $live=@(Get-CimInstance Win32_Process | Select-Object -ExpandProperty ProcessId); foreach($pid in $live){ if($pid -ne $root -and $roots.Contains([int]$pid)){ exit 1 } }; exit 0`;
}
