...
async function runStep(step, index, phase, plan, resourceContext, cancellationAware, captureBudget, status) {
  ...
  return {
    ...
    stdout: step.capture_output === "discard" ? "" : redactManagedJobOutput(raw.stdout, resourceContext, runtimeDir, process.platform, raw.stdoutTruncated),
    stderr: step.capture_output === "discard" ? "" : redactManagedJobOutput(raw.stderr, resourceContext, runtimeDir, process.platform, raw.stderrTruncated),
