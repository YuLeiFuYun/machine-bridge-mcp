...
function remotePublicTool(tool: WorkerToolDefinition): WorkerToolDefinition {
  ...
  if (definition.name === "read_job") {
    ...
    definition.description = `${String(definition.description)} Hosted active reads use a server-side long-poll by default: wait up to ${relayContract.defaultManagedJobReadWaitMs / 1000} seconds for status/phase change or terminal state, returning earlier when progress changes. Set wait_ms=0 only for an immediate checkpoint. A caller may read an active job again in the same assistant response when the current task needs terminal state or further progress. Do not busy-loop or substitute repeated list_jobs calls. ${HOSTED_CONTINUATION_RULE}`;
  }
