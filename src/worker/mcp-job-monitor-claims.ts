...
export class ManagedJobMonitorClaims {
  ...
  issue(jobId: string, authorized: AuthorizedToken, now = Date.now()): string {
    ...
    this.claims.set(key, {
      jobId, monitorId, claimed: false,
      expiresAt: now + JOB_MONITOR_CLAIM_TTL_MS,
  ...
  claim(jobId: string, monitorId: string, authorized: AuthorizedToken, now = Date.now()): Record<string, unknown> | null {
    this.prune(now);
    const key = claimKey(jobId, monitorId, authorized);
    const issued = this.claims.get(key);
    if (!issued) return null;
    this.claims.set(key, { ...issued, claimed: true, expiresAt: now + JOB_MONITOR_CLAIM_TTL_MS, sequence: ++this.sequence });
  ...
  has(jobId: string, monitorId: string, authorized: AuthorizedToken, now = Date.now()): boolean {
    this.prune(now);
    return this.claims.get(claimKey(jobId, monitorId, authorized))?.claimed === true;
  }
